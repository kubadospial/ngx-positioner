...
export class NgxPositionerDirective
  ...
  ngOnInit() {
    ...
    this.positionerService.changeSettings$
  ...
  private _initializeSubscriptions() {
    fromEvent(this._scrollableElement, 'scroll')
      .pipe(takeUntil(this._stopAllSubscriptions$))
    ...
    this._scrollingEvent
      ...
      .subscribe((isScrolledToTop: boolean) =>
        this.isScrolledToTop.next(isScrolledToTop)
      );
  ...
  private _setScrollableElement() {
    ...
    if (!!this._settings.scrollableElement) {
      const scrollEl: HTMLElement = this.document.querySelector(
        this._settings.scrollableElement
      );
      if (!!scrollEl) {
        this._scrollableElement = scrollEl;
      } else {
        console.error(
          `%c Couldn't find ${this._settings.scrollableElement} in DOM! `,
          'color: #fff'
        );
      }
    }
  ...
  private get _isScrolledToBottomCondition(): boolean {
    return (
      this._scrollTop + this._scrollableElement.clientHeight >=
      this._isScrolledToBottomOffset
    );
  }
  ...
  private get _isScrolledToBottomOffset(): number {
    return (
      this._scrollableElement.scrollHeight -
  ...
  private get _moveToBottomOffset(): number {
    return (
      this._scrollableElement.scrollHeight -
      this._scrollableElement.offsetHeight -
